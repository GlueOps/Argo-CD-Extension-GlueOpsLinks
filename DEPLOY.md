...
# 3. Verify
kubectl exec -n argocd $(kubectl get pods -n argocd -l app.kubernetes.io/name=argocd-server -o jsonpath='{.items[0].metadata.name}') -- ls -lh /tmp/extensions/resources/glueops-links-extension/extension.js
```
...
curl -k 'https://localhost:8080/extensions/glueops-links-extension/api/apps/<app-name>/links' \
